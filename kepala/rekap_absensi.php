                ...
                <section class="section">
                    ...
                    <div class="section-body">
                        <?php if ($filter_type == 'daily'): ?>
                            <!-- Ringkasan Ketidakhadiran Harian (Semua Kelas) - Box Tersendiri di Atas Filter -->
                            <div class="row">
                                <div class="col-12">
                                    <div class="card card-statistic-1 border">
                                        <div class="card-header pb-0">
                                            <h4>Ringkasan Ketidakhadiran Seluruh Kelas (<?php echo date('d-m-Y', strtotime($selected_date)); ?>)</h4>
                                        </div>
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <?php for ($day = 1; $day <= 31; $day++): ?>
                                                                        <td>
                                                                            ...
                                                                            if ($is_holiday) {
                                                                                echo '<span style="font-size: 10pt; color: red;" title="' . htmlspecialchars($holidays[$current_date], ENT_QUOTES, 'UTF-8') . '">L</span>';
                                                                            } elseif (!empty($status)) {
                                                                                $status_class = '';
                                                                                switch ($status) {
                                                                                    case 'Hadir': $status_class = 'badge-success'; break;
...
function exportDailyToExcel() {
    ...
    headerDiv.innerHTML += '<h4>Rekap Kehadiran Harian - Tanggal: <?php echo date("d-m-Y", strtotime($selected_date)); ?></h4>';
    headerDiv.innerHTML += '<h4>Tahun Ajaran: ' + academicYear + ' | Semester: ' + activeSemester + '</h4></div><br style="clear: both;">';
    
    var table = document.getElementById('dailyTable');
    if (!table) { alert('Tabel tidak ditemukan'); return; }
    var newTable = table.cloneNode(true);
    
    // Remove badges for Excel
    var badges = newTable.querySelectorAll('.badge');
    for (var i = 0; i < badges.length; i++) {
        var badge = badges[i];
        badge.parentNode.replaceChild(document.createTextNode(badge.textContent.trim()), badge);
    }
...
function exportStudentToExcel() {
    ...
    headerDiv.innerHTML = '<img src="../assets/img/logo_1768301957.png" alt="Logo" style="max-width: 100px; float: left; margin-right: 20px;"><div style="display: inline-block;"><h2>Sistem Informasi Madrasah</h2>';
    headerDiv.innerHTML += '<h3>' + schoolName + '</h3>';
    headerDiv.innerHTML += '<h4>Rekap Kehadiran Siswa: <?php echo addslashes($student_results[0]["nama_siswa"] ?? ""); ?></h4>';
    headerDiv.innerHTML += '<h4>Tahun Ajaran: ' + academicYear + ' | Semester: ' + activeSemester + '</h4></div><br style="clear: both;">';
    ...
    if (typeof XLSX !== 'undefined') {
        ...
        XLSX.writeFile(wb, 'rekap_absensi_siswa_<?php echo $selected_student; ?>.xlsx');
    } else {
        var a = document.createElement('a');
        a.href = 'data:application/vnd.ms-excel;charset=utf-8,' + encodeURIComponent(container.innerHTML);
        a.download = 'rekap_absensi_siswa_<?php echo $selected_student; ?>.xls';
        a.click();
    }
}
...
function fallbackPrintPDF() {
    ...
    printWindow.document.write('<html><head><title>Rekap Kehadiran Bulanan</title>');
    ...
    printWindow.document.write('<h4 style="margin: 0;">Rekap Kehadiran Bulanan - <?php echo htmlspecialchars($js_month_name_safe . " " . $js_month_year_safe, ENT_QUOTES, "UTF-8"); ?></h4></div>');
