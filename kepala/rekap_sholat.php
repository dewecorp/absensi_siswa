...
function exportMonthlyToExcel() {
    ...
    if (typeof XLSX !== 'undefined') {
        ...
        XLSX.writeFile(wb, 'rekap_sholat_berjamaah_<?php echo htmlspecialchars($js_month_name . "_" . $js_month_year, ENT_QUOTES, "UTF-8"); ?>.xlsx');
    } else {
        var a = document.createElement('a');
        var data = 'data:application/vnd.ms-excel;charset=utf-8,' + encodeURIComponent(html);
        a.href = data;
        a.download = 'rekap_sholat_berjamaah.xls';
        a.click();
    }
}
...
function exportMonthlyToPDF() {
    ...
    printWindow.document.write('<h4 style="margin: 4px 0 8px;">Rekap Sholat Berjamaah — <?php echo htmlspecialchars($js_month_name . " " . $js_month_year, ENT_QUOTES, "UTF-8"); ?></h4>');
...
function exportSemesterToExcel() {
    ...
    if (typeof XLSX !== 'undefined') {
        ...
        XLSX.writeFile(wb, 'rekap_sholat_semester_<?php echo htmlspecialchars($active_semester, ENT_QUOTES, "UTF-8"); ?>.xlsx');
    } else {
        // Fallback
        var html = container.innerHTML;
        var a = document.createElement('a');
        var data = 'data:application/vnd.ms-excel;charset=utf-8,' + encodeURIComponent(html);
        a.href = data;
        a.download = 'rekap_sholat_semester.xls';
        a.click();
    }
}
...
function exportDailyToExcel() {
    ...
    var newTable = table.cloneNode(true);
    // Daily table uses badges, we might want to replace them with text
    ...
    if (typeof XLSX !== 'undefined') {
        ...
        XLSX.writeFile(wb, 'rekap_sholat_harian_<?php echo htmlspecialchars($selected_date, ENT_QUOTES, "UTF-8"); ?>.xlsx');
    } else {
        var html = container.innerHTML;
        var a = document.createElement('a');
        var data = 'data:application/vnd.ms-excel;charset=utf-8,' + encodeURIComponent(html);
        a.href = data;
        a.download = 'rekap_sholat_harian.xls';
        a.click();
    }
}
...
function exportDailyToPDF() {
    ...
    printWindow.document.write('<h2 style="margin: 0;">Sistem Informasi Madrasah</h2>');
    printWindow.document.write('<h3 style="margin: 5px 0;"><?php echo htmlspecialchars($school_profile["nama_madrasah"] ?? "Madrasah Ibtidaiyah Negeri Pembina Kota Padang", ENT_QUOTES, "UTF-8"); ?></h3>');
    printWindow.document.write('<h4 style="margin: 4px 0 8px;">Rekap Harian Sholat Berjamaah — <?php echo htmlspecialchars(date("d-m-Y", strtotime($selected_date)), ENT_QUOTES, "UTF-8"); ?></h4>');
    printWindow.document.write('<p style="margin: 0;">Tahun Ajaran: ' + academicYear + ' · ' + semesterLabel + '</p></div>');
    printWindow.document.write('</div>');
...
function exportStudentToExcel() {
    ...
    if (typeof XLSX !== 'undefined') {
        ...
        var data = 'data:application/vnd.ms-excel;charset=utf-8,' + encodeURIComponent(html);
        a.href = data;
        a.download = 'rekap_sholat_siswa.xls';
        a.click();
    }
}
...
function exportStudentToPDF() {
    ...
    printWindow.document.write('<h2 style="margin: 0;">Sistem Informasi Madrasah</h2>');
    printWindow.document.write('<h3 style="margin: 5px 0;"><?php echo htmlspecialchars($school_profile["nama_madrasah"] ?? "Madrasah Ibtidaiyah Negeri Pembina Kota Padang", ENT_QUOTES, "UTF-8"); ?></h3>');
    printWindow.document.write('<h4 style="margin: 4px 0 8px;">Rekap Sholat Berjamaah Siswa: ' + studentName + '</h4>');
    printWindow.document.write('<p style="margin: 0;">Tahun Ajaran: ' + academicYear + ' · ' + semesterLabel + '</p></div>');
    printWindow.document.write('</div>');
