...
$years = [];
try {
    $stmtY = $pdo->query("SELECT DISTINCT YEAR(hari_tanggal) AS thn FROM tb_agenda_kepala WHERE hari_tanggal IS NOT NULL AND hari_tanggal != '0000-00-00' ORDER BY thn DESC");
...
$(document).ready(function () {
    ...
    $('#btn-pdf').on('click', function () {
        ...
        window.open('cetak_agenda_kepala.php?' + params, '_blank');
    });
});
JS;
    ...
    <section class="section">
        ...
        <div class="section-body">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="GET" action="agenda_kepala.php" class="mb-3" id="form-filter">
                            ...
                            <div class="col-md-4 col-6 my-1">
                                <select name="filter_jenis" class="form-control" onchange="this.form.submit()">
                                    ...
                                    <?php foreach ($jenis_list as $j): ?>
                                        <option value="<?= (int)$j['id_jenis'] ?>" <?= (int)$j['id_jenis'] === $filter_jenis ? 'selected' : '' ?>><?= htmlspecialchars($j['jenis_agenda']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <?php if ($filter_tahun !== '' || $filter_jenis > 0): ?>
                            <div class="col-auto my-1">
                                <a href="agenda_kepala.php" class="btn btn-secondary btn-sm"><i class="fas fa-undo"></i> Reset Filter</a>
                            </div>
                            <?php endif; ?>
                        </div>
                    </form>
