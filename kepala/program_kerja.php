...
<div class="row">
<div class="col-md-6"><div class="form-group"><label>Tujuan</label><textarea id="add_tujuan" name="tujuan" class="form-control" rows="3" readonly placeholder="Auto"></textarea></div></div>
<div class="col-md-4"><div class="form-group"><label>Indikator Keberhasilan</label><textarea id="add_indikator" name="indikator" class="form-control" rows="3" readonly placeholder="Auto"></textarea></div></div><div class="col-md-2"><div class="form-group"><label>Target</label><input type="text" id="add_target" name="target" class="form-control" placeholder="Opsional"></div></div>
</div>
...
<div class="col-md-3"><div class="form-group"><label>Evaluasi</label><textarea id="add_evaluasi" name="evaluasi" class="form-control" rows="3" readonly placeholder="Auto"></textarea></div></div>
<div class="col-md-3"><div class="form-group"><label>Tindak Lanjut</label><textarea id="add_tindak" name="tindak_lanjut" class="form-control" rows="3" readonly placeholder="Auto"></textarea></div></div>
</div>
</div>
...
<div class="row">
<div class="col-md-6"><div class="form-group"><label>Tujuan</label><textarea id="edit_tujuan" name="tujuan" class="form-control" rows="3" readonly placeholder="Auto"></textarea></div></div>
<div class="col-md-4"><div class="form-group"><label>Indikator Keberhasilan</label><textarea id="edit_indikator" name="indikator" class="form-control" rows="3" readonly placeholder="Auto"></textarea></div></div><div class="col-md-2"><div class="form-group"><label>Target</label><input type="text" name="target" id="edit_target" class="form-control" placeholder="Opsional"></div></div>
</div>
...
<div class="col-md-3"><div class="form-group"><label>Evaluasi</label><textarea id="edit_evaluasi" name="evaluasi" class="form-control" rows="3" readonly placeholder="Auto"></textarea></div></div>
<div class="col-md-3"><div class="form-group"><label>Tindak Lanjut</label><textarea id="edit_tl" name="tindak_lanjut" class="form-control" rows="3" readonly placeholder="Auto"></textarea></div></div>
</div>
</div>
...
function prokerGrow(scope){ $(scope).find('textarea').each(function(){ this.style.height='auto'; this.style.height=(this.scrollHeight)+'px'; }); }
function syncMatriksAdd(){
 var k=$('#addModal select[name="komponen"]').val(); var prog=$('#add_program_sel').val();
 if(!k || !prog){ $('#add_kegiatan,#add_tujuan,#add_indikator,#add_target').val('');  $('#add_evaluasi,#add_tindak').val(''); prokerGrow('#addModal'); return; }
...
function syncMatriksEdit(){
 ...
 if(!k || !prog){ $('#edit_kegiatan,#edit_tujuan,#edit_indikator,#edit_target').val('');  $('#edit_evaluasi,#edit_tl').val(''); prokerGrow('#editModal'); return; }
 var idx=idxByProgram(k,prog); if(idx<0) return;
 var m=matriks[k][idx]||matriks[String(k)][idx];
...
$(document).on('change','#edit_komponen',function(){ var k=$(this).val(); fillProgram(k,'#edit_program_sel',''); $('#edit_kegiatan,#edit_tujuan,#edit_indikator,#edit_target,#edit_evaluasi,#edit_tl').val('');  });
 ...
 $(document).on('shown.bs.modal','#addModal',function(){ prokerGrow('#addModal'); });
 $(document).on('shown.bs.modal','#editModal',function(){ prokerGrow('#editModal'); });
