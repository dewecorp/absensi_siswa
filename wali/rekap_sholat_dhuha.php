...
if (isset($_POST['action']) && $_POST['action'] == 'update_status') {
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT * FROM tb_sholat_dhuha WHERE id_siswa = ? AND tanggal = ?");
        $stmt->execute([$student_id, $date]);
        
        if ($stmt->rowCount() > 0) {
            $stmt = $pdo->prepare("UPDATE tb_sholat_dhuha SET status = ? WHERE id_siswa = ? AND tanggal = ?");
            $stmt->execute([$status, $student_id, $date]);
        } else {
            $stmt = $pdo->prepare("INSERT INTO tb_sholat_dhuha (id_siswa, tanggal, status) VALUES (?, ?, ?)");
...
if (!$wali_kelas) {
    die('Anda tidak terdaftar sebagai wali kelas manapun.');
}

// Force class_id to wali's class
$class_id = $wali_kelas['id_kelas'];
$classes = [$wali_kelas]; // For dropdown compatibility
...
// Get class information (already verified as wali class)
$class_info = $wali_kelas;
...
if ($class_id > 0) {
    ...
    if ($filter_type == 'daily' && !empty($selected_date)) {
        ...
        // Student filter
        $stmt = $pdo->prepare("SELECT nama_siswa, nisn, k.nama_kelas FROM tb_siswa s JOIN tb_kelas k ON s.id_kelas = k.id_kelas WHERE s.id_siswa = ? AND s.id_kelas = ?");
        $stmt->execute([$selected_student, $class_id]);
...
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>Rekap Sholat Dhuha - Kelas <?php echo htmlspecialchars($wali_kelas['nama_kelas']); ?></h1>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" class="row">
                        <!-- Hidden class_id since wali only has one class -->
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <?php for($m=$start_month; $m<=$end_month; $m++): ?>
                                            ...
                                            <td class="text-center"><?php echo $r['monthly_totals'][$m]['Berhalangan'] ?? 0; ?></td>
                                        <?php endfor; ?>
                                        ...
                                        <td class="text-center font-weight-bold bg-light"><?php echo $r['summary']['Berhalangan'] ?? 0; ?></td>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
...
function exportMonthlyToPDF() {
    ...
    printWindow.document.write('<h2 style="margin: 0;">Sistem Informasi Madrasah</h2>');
    printWindow.document.write('<h3 style="margin: 6px 0;">' + schoolName + '</h3>');
    printWindow.document.write('<h4 style="margin: 4px 0 8px;">Rekap Sholat Dhuha — ' + jsMonthName + ' ' + jsMonthYear + '</h4>');
    printWindow.document.write('<p style="margin: 0;">Tahun Ajaran: ' + academicYear + ' · ' + semesterLabel + '</p>');
    printWindow.document.write('</div>');
    printWindow.document.write('</header>');
...
function exportStudentToPDF() {
    ...
    printWindow.document.write('<h2 style="margin: 0;">Sistem Informasi Madrasah</h2>');
    printWindow.document.write('<h3 style="margin: 6px 0;">' + schoolName + '</h3>');
    printWindow.document.write('<h4 style="margin: 4px 0 8px;">Rekap Sholat Dhuha — ' + studentName + '</h4>');
    printWindow.document.write('<p style="margin: 0;">Tahun Ajaran: ' + academicYear + ' · ' + semesterLabel + '</p>');
    printWindow.document.write('</div>');
    printWindow.document.write('</header>');
