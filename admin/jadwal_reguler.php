...
// If role is wali, get class ID (supports wali_kelas stored as id_guru or nama_guru)
...
$stmt = $pdo->prepare("SELECT * FROM tb_jam_mengajar WHERE jenis = 'Reguler' ORDER BY waktu_mulai ASC");
$stmt->execute();
...
if ($selected_kelas_id) {
    $stmt = $pdo->prepare("
        ...
        LEFT JOIN tb_jam_mengajar jm ON j.jam_ke = jm.jam_ke AND jm.jenis = 'Reguler'
    ...
    foreach ($rows as $row) {
        $class_schedule[$row['hari']][] = $row; // Changed to array list, not keyed by jam_ke to preserve order
    }
}
