    ...
    if (!empty($ids)) {
        try {
            ...
            if (!empty($valid_ids)) {
                ...
                logActivity($pdo, $teacher['nama_guru'], 'Hapus Jurnal Massal', "Guru menghapus $count jurnal");
...
$jam_reguler_stmt = $pdo->prepare("SELECT * FROM tb_jam_mengajar WHERE jenis = 'Reguler' ORDER BY jam_ke ASC");
$jam_reguler_stmt->execute();
$jam_reguler_list = $jam_reguler_stmt->fetchAll(PDO::FETCH_ASSOC);

$jam_ramadhan_stmt = $pdo->prepare("SELECT * FROM tb_jam_mengajar WHERE jenis = 'Ramadhan' ORDER BY jam_ke ASC");
$jam_ramadhan_stmt->execute();
$jam_ramadhan_list = $jam_ramadhan_stmt->fetchAll(PDO::FETCH_ASSOC);

// Combined list for frontend dropdown (tetap mengirim struktur lama ke JavaScript)
$jam_mengajar_list = array_merge($jam_reguler_list, $jam_ramadhan_list);

// Create mapping for time calculation per jenis
$jam_map = [
    'Reguler' => [],
    'Ramadhan' => []
];

foreach ($jam_reguler_list as $jam) {
...
foreach ($jam_ramadhan_list as $jam) {
...
$js_page = [
    ...
    var teacherSchedule = " . json_encode($schedule_map) . ";
    ...
    function openModal(type, data = null) {
        if (type === 'add') {
            ...
            // Then set the value
            // Timeout needed to ensure options are rendered if select2 is used (though standard select is fast)
            setTimeout(function() {
                    ...
                    <div class="card-header">
                        ...
                        <div class="card-header-action">
                            <div class="btn-group mr-2">
                                <a href="../config/export_jurnal_pdf?session_type=guru&kelas=<?php echo $_GET['kelas'] ?? ''; ?>&jenis=<?php echo $_GET['jenis'] ?? ''; ?>&bulan=<?php echo $_GET['bulan'] ?? ''; ?>" target="_blank" class="btn btn-danger">
                                    <i class="fas fa-file-pdf"></i> Export PDF
                                </a>
                                <a href="../config/export_jurnal_excel?session_type=guru&kelas=<?php echo $_GET['kelas'] ?? ''; ?>&jenis=<?php echo $_GET['jenis'] ?? ''; ?>&bulan=<?php echo $_GET['bulan'] ?? ''; ?>" target="_blank" class="btn btn-success">
