...
foreach ($grade_headers as $header) {
    ...
    // Merge for Activity & Date (New Row)
    $kegiatan = isset($header['jenis_kegiatan']) ? $header['jenis_kegiatan'] : '-';
    $tanggal = isset($header['tgl_kegiatan']) ? $header['tgl_kegiatan'] : '-';
    $subHeader = $kegiatan . ' (' . $tanggal . ')';
    
    $sheet->setCellValue($currentCol . ($row + 1), $subHeader);
...
// Output
$filename = 'Nilai_Kokurikuler_' . str_replace(' ', '_', $class_info['nama_kelas']) . '_' . str_replace(' ', '_', $mapel_info['nama_mapel']) . '_' . date('Y-m-d') . '.xlsx';
